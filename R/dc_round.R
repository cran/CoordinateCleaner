dc_round <- function(x, lon = "decimallongitude", lat = "decimallatitude", ds = "dataset", 
    ...
    if (length(unique(x[[ds]])) > 1) {
        ...
        out <- lapply(dat, function(k) {
            ...
            if (nrow(tester[!duplicated(tester[, c(lon, lat)]), ]) < min.unique.ds.size) {
                ...
                if (test == "lon") {
                  ...
                  gvec <- .CalcACT(data = k[[lon]], digit.round = digit.round, 
                ...
                if (test == "lat") {
                  gvec <- .CalcACT(data = k[[lat]], digit.round = digit.round, 
                    nc = nc, graphs = graphs, graph.title = unique(k[[ds]]))
                  # run the sliding window outlier detection
                  n.outl <- .OutDetect(gvec, T1 = T1, window.size = window.size, 
                    detection.rounding = detection.rounding, detection.threshold = detection.threshold, 
                ...
                if (test == "both") {
                  gvec1 <- .CalcACT(data = k[[lon]], digit.round = digit.round, 
                    nc = nc, graphs = graphs, graph.title = unique(k[[ds]]))
                  n.outl.lon <- .OutDetect(gvec1, T1 = T1, window.size = window.size, 
                    detection.rounding = detection.rounding, detection.threshold = detection.threshold, 
                    graphs = graphs)
                  
                  n.outl.lon$flag <- !all(n.outl.lon$n.outliers > 0, n.outl.lon$regular.distance >= 
                  ...
                  gvec2 <- .CalcACT(data = k[[lat]], digit.round = digit.round, 
                    nc = nc, graphs = graphs, graph.title = unique(k[[ds]]))
                  n.outl.lat <- .OutDetect(gvec2, T1 = T1, window.size = window.size, 
                    detection.rounding = detection.rounding, detection.threshold = detection.threshold, 
                    graphs = graphs)
                  
                  n.outl.lat$flag <- !all(n.outl.lat$n.outliers > 0, n.outl.lat$regular.distance >= 
        ...
        if (nrow(x[!duplicated(x[, c(lon, lat)]), ]) < min.unique.ds.size) {
            warning("Dataset smaller than minimum test size")
            out <- data.frame(dataset = unique(x[[ds]]), n.outliers = NA, n.regular.outliers = NA, 
                regular.distance = NA, summary = NA)
        } else {
            if (test == "lon") {
                # calculate autocorrelation
                gvec <- .CalcACT(data = x[[lon]], digit.round = digit.round, 
            ...
            if (test == "lat") {
                gvec <- .CalcACT(data = x[[lat]], digit.round = digit.round, 
                  nc = nc, graphs = graphs, graph.title = unique(x[[ds]]))
                # run the sliding window outlier detection
                n.outl <- .OutDetect(gvec, T1 = T1, window.size = window.size, 
                  detection.rounding = detection.rounding, detection.threshold = detection.threshold, 
            ...
            if (test == "both") {
                gvec1 <- .CalcACT(data = x[[lon]], digit.round = digit.round, 
                  nc = nc, graphs = graphs, graph.title = unique(x[[ds]]))
                ...
                gvec2 <- .CalcACT(data = x[[lat]], digit.round = digit.round, 
                  nc = nc, graphs = graphs, graph.title = unique(x[[ds]]))
